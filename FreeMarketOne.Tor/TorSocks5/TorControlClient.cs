...
namespace FreeMarketOne.Tor
{
    public sealed class TorControlClient
    {
        ...
        public async Task Init(CancellationToken ctsToken = default)
        {
            if (!initialised)
            {
        ...
        {
            return await SendCommandAsync(command, initAuthDispose: false, ctsToken: ctsToken).ConfigureAwait(false);
        }
